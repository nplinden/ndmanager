...
class ResonanceCovariances(Resonances):
    ...
    Parameters
    ----------
    ranges : list of ndmanager._vendor.omc_data.ResonanceCovarianceRange
    ...
    def ranges(self, ranges) -> None:
        ...
        self._ranges = cv.CheckedList(ResonanceCovarianceRange, "resonance range", ranges)
    ...
    def from_endf(cls, ev, resonances):
        ...
        ev : ndmanager._vendor.omc_data.endf.Evaluation
            ENDF evaluation
        resonances : ndmanager._vendor.omc_data.Resonance object
            ndmanager._vendor.omc_data.Resonanance object generated from the same evaluation
            used to import values not contained in File 32

        Returns
        -------
        ndmanager._vendor.omc_data.ResonanceCovariances
        ...
        for _ in range(n_isotope):
            ...
            for j in range(n_ranges):
                ...
                if formalism in [0, 7]:
                    error = "LRF=" + str(formalism) + " covariance not supported " "for this formalism"
                    raise NotImplementedError(error)

                if unresolved_flag in (0, 1):
                    # Resolved resonance region
                    resonance = resonances.ranges[j]
                    erange = _FORMALISMS[formalism].from_endf(ev, file_obj, items, resonance)
                    ranges.append(erange)

                elif unresolved_flag == 2:
                    warn = (
                        "Unresolved resonance not supported. Covariance "
...
class ResonanceCovarianceRange:
    ...
    def subset(self, parameter_str, bounds):
        ...
        Returns
        -------
        res_cov_range : ndmanager._vendor.omc_data.ResonanceCovarianceRange
    ...
    def sample(self, n_samples):
        ...
        Returns
        -------
        samples : list of ndmanager._vendor.omc_data.ResonanceCovarianceRange objects
            List of samples size `n_samples`

        """
        warn_str = (
            "Sampling routine does not guarantee positive values for "
        ...
        if formalism in ("mlbw", "slbw"):
            params = ["energy", "neutronWidth", "captureWidth", "fissionWidth", "competitiveWidth"]
            ...
            for sample in par_samples:
                ...
                for j, E in enumerate(energy):
                    records.append([E, l_value[j], spin[j], gt[j], gn[j], gg[j], gf[j], gx[j]])
                columns = [
                    "energy",
                    "L",
                    "J",
        ...
        # Handling RM sampling
            ...
            params = ["energy", "neutronWidth", "captureWidth", "fissionWidthA", "fissionWidthB"]
            param_list = params[:mpar]
            mean_array = parameters[param_list].values
            mean = mean_array.flatten()
            par_samples = rng.multivariate_normal(mean, cov, size=n_samples)
            ...
            for sample in par_samples:
                ...
                records = []
                for j, E in enumerate(energy):
                    records.append([E, l_value[j], spin[j], gn[j], gg[j], gfa[j], gfb[j]])
...
class MultiLevelBreitWignerCovariance(ResonanceCovarianceRange):
    ...
    def __init__(self, energy_min, energy_max, parameters, covariance, mpar, lcomp, file2res) -> None:
    ...
    def from_endf(cls, ev, file_obj, items, resonance):
        ...
        items : list
            ...
            subsection
        resonance : ndmanager._vendor.omc_data.ResonanceRange object
            Corresponding resonance range with File 2 data.

        Returns
        -------
        ndmanager._vendor.omc_data.MultiLevelBreitWignerCovariance
        ...
        if lcomp == 1:
            ...
            for i in range(num_short_range):
                ...
                for i, E in enumerate(energy):
                    records.append([E, spin[i], gt[i], gn[i], gg[i], gf[i]])
            ...
            for i in range(nls):
                ...
                for j in range(num_res):
                    ...
                    if j < num_res - 1:  # Pad matrix for additional values
                        cov = np.pad(cov, ((0, 4), (0, 4)), "constant", constant_values=0)

        # Create pandas DataFrame with resonance data, currently
        # redundant with data.IncidentNeutron.resonance
        columns = ["energy", "J", "totalWidth", "neutronWidth", "captureWidth", "fissionWidth"]
        ...
        parameters = _add_file2_contributions(parameters, resonance.parameters)
        # Create instance of class
        return cls(energy_min, energy_max, parameters, cov, mpar, lcomp, resonance)
...
class SingleLevelBreitWignerCovariance(MultiLevelBreitWignerCovariance):
    ...
    def __init__(self, energy_min, energy_max, parameters, covariance, mpar, lcomp, file2res) -> None:
        super().__init__(energy_min, energy_max, parameters, covariance, mpar, lcomp, file2res)
        self.formalism = "slbw"
...
class ReichMooreCovariance(ResonanceCovarianceRange):
    ...
    def from_endf(cls, ev, file_obj, items, resonance):
        ...
        items : list
            Items from the CONT record at the start of the resonance range
            subsection
        resonance : ndmanager._vendor.omc_data.Resonance object
            ndmanager._vendor.omc_data.Resonanance object generated from the same evaluation
            used to import values not contained in File 32

        Returns
        -------
        ndmanager._vendor.omc_data.ReichMooreCovariance
        ...
        if lcomp == 1:
            ...
            for i in range(num_short_range):
                ...
                for i, E in enumerate(energy):
                    records.append([E, spin[i], gn[i], gg[i], gfa[i], gfb[i]])
        ...
        columns = ["energy", "J", "neutronWidth", "captureWidth", "fissionWidthA", "fissionWidthB"]
        parameters = pd.DataFrame.from_records(records, columns=columns)
