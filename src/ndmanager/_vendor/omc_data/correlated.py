...
class CorrelatedAngleEnergy(AngleEnergy):
    ...
    energy_out : Iterable of ndmanager._vendor.omc_data.stats.Univariate
        Distribution of outgoing energies corresponding to each incoming energy
    mu : Iterable of Iterable of ndmanager._vendor.omc_data.stats.Univariate
    ...
    def breakpoints(self, breakpoints) -> None:
        cv.check_type("correlated angle-energy breakpoints", breakpoints, Iterable, Integral)
    ...
    def interpolation(self, interpolation) -> None:
        cv.check_type("correlated angle-energy interpolation", interpolation, Iterable, Integral)
    ...
    def energy_out(self, energy_out) -> None:
        cv.check_type("correlated angle-energy outgoing energy", energy_out, Iterable, Univariate)
    ...
    @mu.setter
    def mu(self, mu) -> None:
        cv.check_iterable_type("correlated angle-energy outgoing cosine", mu, Univariate, 2, 2)
        self._mu = mu
    ...
    def to_hdf5(self, group) -> None:
        ...
        for i, mu_i in enumerate(self.mu):
            mu_tabular.append(
                [mu_ij if isinstance(mu_ij, Tabular | Discrete) else mu_ij.to_tabular() for mu_ij in mu_i]
            )
        ...
        for i, d in enumerate(self.energy_out):
            ...
            if isinstance(d, Mixture):
                ...
                if isinstance(d, Tabular):
                    ...
                    msg = "Invalid univariate energy distribution as part of " f"correlated angle-energy: {d}"
    ...
    def from_hdf5(cls, group):
        ...
        Returns
        -------
        ndmanager._vendor.omc_data.CorrelatedAngleEnergy
            Correlated angle-energy distribution
        ...
        for i in range(n_energy):
            ...
            for j in range(n):
                ...
                if interp_code == 0:
                    ...
                    mu_ij = Tabular(x, p, INTERPOLATION_SCHEME[interp_code], ignore_negative=True)
                mu_ij.c = c
                mu_i.append(mu_ij)
    ...
    def from_ace(cls, ace, idx, ldis):
        ...
        for i in range(n_energy_in):
            ...
            if np.any(data[1][n_discrete_lines:] < 0.0):
                warn("Correlated angle-energy distribution has negative " "probabilities.")
    ...
    def from_endf(cls, file_obj):
        ...
        for i in range(ne):
            ...
            eout_i = values[:, 0]
            eout_p_i = values[:, 1]
            energy_out_i = Tabular(eout_i, eout_p_i, INTERPOLATION_SCHEME[lep], ignore_negative=True)
