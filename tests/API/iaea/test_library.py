...
def test_parse_index():
    ...
    assert (
        kwargs["url"] == "https://www-nds.iaea.org/public/download-endf/IAEA-Medical/"
    )
    assert kwargs["lib"] == "IAEA-Medical"
    assert (
        kwargs["library"]
        == "IAEA-Medical Charged-particle cross section database for medical radioisotope production, 2001"
    )
