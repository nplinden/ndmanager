...
def test_photon_manager(install, build_lib):
    ...
    photo = {
        "C": "pytest-artifacts/endf6/foo/photo/C.endf6",
    ...
    H = [m for m in manager if m.target == "H"][0]
    assert H.target == "H"
    assert H.path == Path("pytest-artifacts/API/process/neutron_manager/photon/H.h5")
    assert H.logpath == Path(
        "pytest-artifacts/API/process/neutron_manager/photon/logs/H.log"
    )
