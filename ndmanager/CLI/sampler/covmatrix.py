...
class NdsCovCommand:
    ...
    def parser(cls, subparsers: ap._SubParsersAction):
        parser = subparsers.add_parser("cov", 
                                       help="Build covariance matrices using Sandy")
        parser.add_argument(
            "library",
            type=str,
            help="The library for which to build the matrices"
        )
        parser.add_argument(
            "--ign",
            type=str,
...
def generate_matrices(library: str, ign: str, clean: bool, processes: int):
    ...
    if directory.exists() and not clean:
        raise FileExistsError("This covariance library already exists"
    ...
    with mp.get_context("spawn").Pool(processes) as p:
        for nuclide in IAEA()[library]["n"].keys():
            p.apply_async(generate_one_matrix,
                          args=(library, nuclide, ign_value, directory))
        p.close()
        p.join()
    return failed
...
def generate_one_matrix(library, nuclide, ign_value, directory):
        ...
        try:
            matrix = CovMatrix.from_tape(tape, 
                                         ign=ign_value, 
