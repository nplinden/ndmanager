...
class Resonances:
    ...
    Parameters
    ----------
    ranges : list of ndmanager._vendor.omc_data.ResonanceRange
    ...
    ranges : list of ndmanager._vendor.omc_data.ResonanceRange
        Distinct energy ranges for resonance data
    resolved : ndmanager._vendor.omc_data.ResonanceRange or None
        Resolved resonance range
    unresolved : ndmanager._vendor.omc_data.Unresolved or None
    ...
    def ranges(self, ranges) -> None:
        ...
        self._ranges = cv.CheckedList(ResonanceRange, "resonance ranges", ranges)

    @property
    def resolved(self):
        resolved_ranges = [r for r in self.ranges if not isinstance(r, Unresolved)]
    ...
    def from_endf(cls, ev):
        ...
        Parameters
        ----------
        ev : ndmanager._vendor.omc_data.endf.Evaluation
            ENDF evaluation
...
class ResonanceRange:
    ...
    def from_endf(cls, ev, file_obj, items):
        ...
        Returns
        -------
        ndmanager._vendor.omc_data.ResonanceRange
        ...
        a = Polynomial((0.123 * (NEUTRON_MASS * ev.target["mass"]) ** (1.0 / 3.0) + 0.08,))
...
class MultiLevelBreitWigner(ResonanceRange):
    ...
    def __init__(self, target_spin, energy_min, energy_max, channel, scattering) -> None:
        super().__init__(target_spin, energy_min, energy_max, channel, scattering)
    ...
    def from_endf(cls, ev, file_obj, items):
        ...
        Returns
        -------
        ndmanager._vendor.omc_data.MultiLevelBreitWigner
        ...
        for _l in range(NLS):
            ...
            # Calculate channel radius from ENDF-102 equation D.14
            a = Polynomial((0.123 * (NEUTRON_MASS * awri) ** (1.0 / 3.0) + 0.08,))
            ...
            for i, E in enumerate(energy):
                records.append([E, l_value, spin[i], gt[i], gn[i], gg[i], gf[i], gx[i]])

        columns = ["energy", "L", "J", "totalWidth", "neutronWidth", "captureWidth", "fissionWidth", "competitiveWidth"]
        parameters = pd.DataFrame.from_records(records, columns=columns)

        # Create instance of class
        mlbw = cls(target_spin, energy_min, energy_max, channel_radius, scattering_radius)
...
class ReichMoore(ResonanceRange):
    ...
    def from_endf(cls, ev, file_obj, items):
        ...
        Returns
        -------
        ndmanager._vendor.omc_data.ReichMoore
        ...
        for i in range(NLS):
            ...
            for i, E in enumerate(energy):
                records.append([E, l_value, spin[i], gn[i], gg[i], gfa[i], gfb[i]])

        # Create pandas DataFrame with resonance data
        columns = ["energy", "L", "J", "neutronWidth", "captureWidth", "fissionWidthA", "fissionWidthB"]
        parameters = pd.DataFrame.from_records(records, columns=columns)

        # Create instance of ReichMoore
        rm = cls(target_spin, energy_min, energy_max, channel_radius, scattering_radius)
    ...
    def _prepare_resonances(self) -> None:
        ...
        for l, J in lj_values:
            self._parameter_matrix[l, J] = df[(l == df.L) & (abs(df.J) == J)].values
...
class RMatrixLimited(ResonanceRange):
    ...
    def from_endf(cls, ev, file_obj, items):
        ...
        Returns
        -------
        ndmanager._vendor.omc_data.RMatrixLimited
        ...
        for i in range(n_pairs):
            ...
            particle_pairs.append(ParticlePair(first, second, q_value, penetrability, shift, mt))
        ...
        for i in range(n_spin_groups):
            ...
            for j in range(n_channels):
                channel = {}
                channel["particle_pair"] = particle_pairs[int(values[6 * j]) - 1]
            ...
            for j in range(n_resonances):
                energy = values[6 * m * j]
                records.append([energy] + [values[6 * m * j + k + 1] for k in range(n_channels)])
...
class ParticlePair:
    def __init__(self, first, second, q_value, penetrability, shift, mt) -> None:
        self.first = first
        self.second = second
...
class SpinGroup:
    ...
    parity : {'+', '-'}
        Even (+) or odd(-) parity
    channels : list of ndmanager._vendor.omc_data.Channel
        Available channels
    parameters : pandas.DataFrame
...
class Unresolved(ResonanceRange):
    ...
    channel : ndmanager._vendor.omc_data.Function1D
        Channel radii as a function of energy
    scattering : ndmanager._vendor.omc_data.Function1D
    ...
    channel_radius : ndmanager._vendor.omc_data.Function1D
        Channel radii as a function of energy
    ...
    scattering_radius : ndmanager._vendor.omc_data.Function1D
        Scattering radii as a function of energy
    target_spin : float
        Intrinsic spin, :math:`I`, of the target nuclide
    ...
    def from_endf(cls, file_obj, items, fission_widths):
        ...
        Returns
        -------
        ndmanager._vendor.omc_data.Unresolved
        ...
        if not fission_widths and formalism == 1:
            ...
            NLS = items[4]
            columns = ["L", "J", "E", "d", "amux", "amun", "amuf", "gx", "gn0", "gg", "gf"]
            records = []
            for _ls in range(NLS):
                items = get_cont_record(file_obj)
                awri = items[0]
                ...
                for j in range(NJS):
                    ...
                    for k in range(1, ne + 1):
                        ...
                        gf = values[6 * k + 5]
                        energies.append(E)
                        records.append([l, j, E, d, amux, amun, amuf, gx, gn0, gg, gf])
            parameters = pd.DataFrame.from_records(records, columns=columns)
        ...
        urr = cls(target_spin, energy_min, energy_max, channel_radius, scattering_radius)
...
_FORMALISMS = {0: ResonanceRange, 1: SingleLevelBreitWigner, 2: MultiLevelBreitWigner, 3: ReichMoore, 7: RMatrixLimited}

_RESOLVED = (SingleLevelBreitWigner, MultiLevelBreitWigner, ReichMoore, RMatrixLimited)
