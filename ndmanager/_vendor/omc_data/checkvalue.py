...
def check_type(name, value, expected_type, expected_iter_type=None, *, none_ok=False) -> None:
    ...
    if not isinstance(value, expected_type):
        if isinstance(expected_type, Iterable):
            ...
            msg = f'Unable to set "{name}" to "{value}" which is not of type "' f'{expected_type.__name__}"'
        raise TypeError(msg)
        ...
        for item in value:
            if not isinstance(item, expected_iter_type):
                if isinstance(expected_iter_type, Iterable):
                    msg = (
                        'Unable to set "{}" to "{}" since each item must be ' 'one of the following types: "{}"'.format(
    ...
    while index[0] != len(tree[0]):
        ...
        if isinstance(current_item, expected_type):
            ...
            if len(tree) < min_depth:
                msg = (
                    f'Error setting "{name}": The item at {ind_str} does not ' f"meet the minimum depth of {min_depth}"
                )
                raise TypeError(msg)
        ...
        elif isinstance(current_item, Iterable):
...
def check_length(name, value, length_min, length_max=None) -> None:
    ...
    if length_max is None:
        if len(value) < length_min:
            msg = f'Unable to set "{name}" to "{value}" since it must be at ' f'least of length "{length_min}"'
            raise ValueError(msg)
    elif not length_min <= len(value) <= length_max:
        if length_min == length_max:
            msg = f'Unable to set "{name}" to "{value}" since it must be of ' f'length "{length_min}"'
        else:
            msg = (
...
def check_increasing(name: str, value, equality: bool = False) -> None:
    ...
    if equality:
        if not np.all(np.diff(value) >= 0.0):
            msg = f'Unable to set "{name}" to "{value}" since its ' "elements must be increasing."
            raise ValueError(msg)
    elif not equality and not np.all(np.diff(value) > 0.0):
        msg = f'Unable to set "{name}" to "{value}" since its ' "elements must be strictly increasing."
        raise ValueError(msg)
...
def check_value(name, value, accepted_values) -> None:
    ...
    if value not in accepted_values:
        msg = f'Unable to set "{name}" to "{value}" since it is not in ' f'"{accepted_values}"'
        raise ValueError(msg)
...
def check_less_than(name, value, maximum, equality=False) -> None:
    ...
    if equality:
        if value > maximum:
            msg = f'Unable to set "{name}" to "{value}" since it is greater ' f'than "{maximum}"'
            raise ValueError(msg)
    elif value >= maximum:
        msg = f'Unable to set "{name}" to "{value}" since it is greater ' f'than or equal to "{maximum}"'
        raise ValueError(msg)
...
def check_greater_than(name, value, minimum, equality=False) -> None:
    ...
    if equality:
        if value < minimum:
            msg = f'Unable to set "{name}" to "{value}" since it is less than ' f'"{minimum}"'
            raise ValueError(msg)
    elif value <= minimum:
        msg = f'Unable to set "{name}" to "{value}" since it is less than ' f'or equal to "{minimum}"'
        raise ValueError(msg)
...
def check_filetype_version(obj, expected_type, expected_version) -> None:
    ...
    try:
        ...
        if this_version[0] != expected_version:
            msg = (
                "{} file has a version of {} which is not " "consistent with the version expected by OpenMC, {}".format(
